...
3. **НЕ распаковывайте в `C:\Program Files\`** — приложение хранит логи и историю транскрипций рядом с собой, а в Program Files обычному пользователю Windows писать нельзя.
...
4. Вставьте ключ в Настройки → Облачный провайдер → API key (для AssemblyAI).
...
4. Нажмите **«Извлечь задачи»** — откроется отдельный диалог.
   ...
   - Нажмите «Извлечь».
5. Через 30-60 секунд в папке `history/<метка времени>_<имя файла>/` появятся:
   - `transcript.txt` — текст транскрипции
...
6. **Кнопка «История»** в основном окне — открывает список всех прошлых транскрипций. Двойной клик загружает её обратно в окно (можно перезапустить извлечение задач с другими параметрами).
...
- История транскрипций хранится локально в `history/` рядом с exe — никуда не уходит.
